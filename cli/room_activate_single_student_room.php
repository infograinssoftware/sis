...
foreach($rooms as $r)
{
	...
	//get the facility_id
	$facility = rc_ps_get_facility($facility_id);
	if($facility)
	{
		if($facility['EFF_STATUS'] == 'I')
		{
			$trace->output($r['FACILITY_ID'] . ' => ' . $r['EFF_STATUS']);		
			//make it active
//			rc_ps_make_room_active($facility_id, 'A');
//			$facility = rc_ps_get_facility($facility_id);
//			$trace->output($facility['FACILITY_ID'] . ' => ' . $facility['EFF_STATUS']);		
			$processed++;
//			break;
		}
	}
	else
		$trace->output($facility_id . ' Not found');
	$count++;
}

$trace->output('Total records checked : ' . $count);
$trace->output('Total inactive room : ' . $processed);
