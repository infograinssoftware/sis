...
require_once($CFG->dirroot.'/course/format/rcyci/attendance/lib.php');

$trace->output('The process may take some time. Please be patient.....');

$trace->output('');

//first, we update the excuses
$trace->output('Updating excuses');
rc_format_attendance_excuse_update_all();

$trace->output('Generate the DN list for week ');

//delete the existing DN list

$now = time();
$current_week = rc_format_attendance_get_current_week($now);
$cut_off = $current_week - 1;

$date_list = rcyci_format_attendance_week_date($cut_off);
$transaction_date = $date_list['Monday']['date_php']; //transaction date is on Monday of each academic week

$DB->delete_records('si_dn_list', array('strm' => $CFG->semester, 'cut_off_week' => $cut_off));

//$trace->output('current_week : ' . $current_week);
//$trace->output('cut_off : ' . $cut_off);

//get all the theory courses
$count = 0;
$rs = rc_ps_get_all_class('', true);
while(!$rs->EOF) 
{
	$rec = $rs->fields;
	if($course = $DB->get_record('course', array('idnumber' => $rec['CLASS_NBR'])))
	{
		$trace->output('Processing DN list for ' . $course->shortname . ' (' . $course->idnumber . ')');
		rcyci_format_attendance_process_dn($course, $current_week, $cut_off, $transaction_date, true);
		$count++;
	}
	$rs->MoveNext();
}
