...
//$rooms = rc_ps_room_single_student();
$rooms = rc_ps_get_dormitory();
$count = 0;
$processed = 0;
$timestamp = time();
//rc_delete_dormitory(); //first we delete all the recorded status

foreach($rooms as $r)
{
	$facility_id = $r['FACILITY_ID'];
	$status = $r['EFF_STATUS'];
	rc_save_dormitory($facility_id, $status, $timestamp);
	if($status == 'I') //change the status accordingly
	{
		$trace->output($r['FACILITY_ID'] . ' => ' . $r['EFF_STATUS']);		
		//make it active
		rc_ps_make_room_active($facility_id, 'A'); //change the status accordingly. A = active, I = inactive. Must be reverse from above
		$processed++;
	}
	$count++;
}

$trace->output('Total records checked : ' . $count);
$trace->output('Total records activated : ' . $processed);
