...
$trace->output('Deleting log');

$sql = "select min(id) as id from {logstore_standard_log}";
$rec = $DB->get_record_sql($sql);
$min = $rec->id;
$sql = "select max(id) as id from {logstore_standard_log}";
$rec = $DB->get_record_sql($sql);
$max = $rec->id;

$trace->output($min . ' - ' . $max);
$i = $min + 10000;

while($i <= $max)
{
	$sql = "delete from {logstore_standard_log} where id <= $i";
	$DB->execute($sql);
	$trace->output($i);
	$i = $i + 10000;
}
