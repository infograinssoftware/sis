...
$trace->output('');
$trace->output('Synchronize attendance record in Moodle and Peoplesoft');
...
$week_day = rcyci_format_attendance_week_date($cut_off);
$last_day = $week_day['Saturday'];
//get all the section in peoplesoft
$rec = rc_ps_get_all_class();
foreach($rec as $r)
{
	$trace->output('Processing ' . $r['SUBJECT'] . $r['CATALOG_NBR'] . ' ' . $r['CLASS_SECTION'] .  ' (' . $r['CLASS_NBR'] . ')');
	rc_format_attendance_transfer_ps($r['CLASS_NBR'], $last_day, null); //for all, we pass null as first day
}

//rc_format_attendance_transfer_ps_all();
