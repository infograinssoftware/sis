...
$users = rc_ps_room_get_ci_student();
$count = 0;
$inactive = 0;
foreach($users as $u)
{
	if(!rc_ps_is_active($u['EMPLID']))
	{
		rc_ps_room_checkout_student($u['EMPLID']);
		$trace->output($u['EMPLID'] . ' => ' . $u['FACILITY_ID']);
		$inactive++;
	}
	$count++;
}
...
$trace->output('Total inactive student in room : ' . $inactive);
