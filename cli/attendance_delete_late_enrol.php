...
$count = 0;
//get all the active student in peplesoft
$records = rc_ps_get_all_enrol($CFG->semester);
foreach($records as $rec)
{
	$emplid = $rec['EMPLID'];
	$class_nbr = $rec['CLASS_NBR'];
	$action_date = strtotime($rec['ENRL_ADD_DT']);
	//get the attendance record for the student and his class_alias
	$sql = "select * from {si_absence} where strm = '$CFG->semester' and emplid = '$emplid' and class_nbr = '$class_nbr'";
	$absences = $DB->get_records_sql($sql);
	if($absences)
	{
		foreach($absences as $absence)
		{
			if($absence->class_attend_dt_php <= $action_date)
			{
				$DB->delete_records('si_absence', array('id' => $absence->id));
				$trace->output('Delete: ' . $absence->id .
								' EMPLID: ' . $absence->emplid . 
								' c_nbr: ' . $absence->class_nbr . 
								' Abs date : ' . date('d-M-Y', $absence->class_attend_dt_php) . 
								' Enrl date : ' . date('d-M-Y', $action_date)
								);
				$count++;
			}
		}
