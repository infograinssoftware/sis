...
require_once($CFG->dirroot.'/local/rcyci/timetable/lib.php');

$trace->output('The process may take some time. Please be patient.....');

$trace->output('');

$users = rc_ps_get_all_user('student');
$total = 0;
$count = 0;
foreach($users as $u)
{
	$conflict = rcyci_timetable_timetable_conflict($u['EMPLID']);
	if($conflict)
	{
		$trace->output($u['EMPLID'] . ' ' . $u['INSTITUTE']);		
		$count++;
	}
	$total++;
}

$trace->output($total . ' students checked');
$trace->output($count . ' with schedule conflict');
$trace->output('');
