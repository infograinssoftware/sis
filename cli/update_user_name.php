...
$users = rc_ps_get_all_user();
$count = 1;
foreach($users as $u)
{
	$m_u = $DB->get_record('user', array('idnumber' => $u['EMPLID']));
	if($m_u)
	{
		$m_u->firstname = $u['FIRST_NAME'];
		$m_u->lastname = $u['LAST_NAME'];
		if(isset($u['EMAIL']))
			$m_u->email = $u['EMAIL'];
		$DB->update_record('user', $m_u);
		$trace->output($count . ' Updated ' . $m_u->firstname . ' ' . $m_u->lastname);		
		$count++;
	}
}
