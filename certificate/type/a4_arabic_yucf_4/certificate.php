...
if($student_id == 1) //dummy for preview
{
	...
	$student->firstname = 'شهد عبدالله الذبياني';
	$student->lastname = '';
	$student_name = $student->firstname;
	$student_id = '12121245';
	$serial_no = '#000001';
	$award_num = $cert->award_date;
}
else
{
...
//	$student = $DB->get_record('user', array('username' => $student_id));
	...
	if($student_rec && $student_rec->custom_name != '')
		...
		$student_name = ucwords($student_data['A_OFFICIAL_NAME']);
	if($student_rec && $student_rec->custom_award_date != '')
		$award_num = $student_rec->custom_award_date;
	else
		$award_num = $cert->award_date;
...
certificate_print_text($pdf, $x + 30, $y + 60, 'C', $font, '', 30, $certificate->recepient);
//certificate_print_text($pdf, $x, $y + 55, 'C', $font, '', 20, $student_id);
//certificate_print_text($pdf, $x + 60, $y + 58, '', $font, '', 20, 'Has Completed ');
//certificate_print_text($pdf, $x + 130, $y + 89, '', $font, 'C', 20, $certificate->name);
certificate_print_text($pdf, $x + 201, $y + 84, '', $font, '', 20, $award_num);
...
//certificate_print_text($pdf, $x + 75, $y + 91, '', $font, '', 15, $certificate->cert_date);
